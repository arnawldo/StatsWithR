...
```{r set-seed}
set.seed(14101992)                 # make sure to change the seed
```
...
ggplot(samp, aes(x=area)) + geom_histogram(binwidth = 150)

#summary
samp %>%
  summarise(x_bar=mean(area), median_dd=median(area), std_dd=sd(area))
```
The distribution is right skewed, which is expected becuase areas can't be negative but there's no limit to how big it can get. The typical size is about 1500, from the median, which is more dpendable measure in this right skewed distribution.
