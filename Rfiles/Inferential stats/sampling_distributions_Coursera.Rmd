...
samp1 %>%
  summarise(mean_samp=mean(area), median_samp=median(area), sd_samp=sd(area), iqr_samp=IQR(area), min_samp=min(area), max_samp=max(area), q1_samp=quantile(area, 0.25), q3_samp=quantile(area, 0.75))
```
...
dim(sample_means50)
```
There are 15000 elements in the sampling distribution. 
```{r}
#summary of distribution
sample_means50 %>%
  summarise(mean_samp=mean(x_bar), median_samp=median(x_bar), sd_samp=sd(x_bar), iqr_samp=IQR(x_bar), min_samp=min(x_bar), max_samp=max(x_bar), q1_samp=quantile(x_bar, 0.25), q3_samp=quantile(x_bar, 0.75))
```
The sampling distribution has a mean of 1499.677 which is pretty close to the population mean.
```{r}
...
```
Each observation indicates a sample mean. The mean of the sampling distribution is closer to population mean as sample size increases but barely changes with increase in simulations.
The standard error decreases with increase in sample size but mostly unchanged with increase in simulations.
...
ggplot(sample_means50, aes(x=x_bar)) + geom_histogram(binwidth = 2000)
```
Sampling distribution of mean price is normal with mean about $180000.
...
ggplot(sample_means150, aes(x=x_bar)) + geom_histogram(binwidth = 2000)
```
The sampling distribution is normal with narrower curve than sampling_ means50. I would still guess $180000 to be the true mean.
* * *
...
mean(samp3$price)
```
My best point estimate of mean home prices is $170000
...
ggplot(sample_means15, aes(x=x_bar)) + geom_histogram(binwidth = 3000)
```
My best guess for population mean would be little over $175000
```{r}
#population mean for price
mean(ames$price)
```
...
ggplot(sample_means150, aes(x=x_bar)) + geom_histogram(binwidth = 3000)
```
My best guess would be $180000
...
#smaple_means50
sample_means15 %>%
  summarise(mean_samp=mean(x_bar), sd_samp=sd(x_bar))
```
