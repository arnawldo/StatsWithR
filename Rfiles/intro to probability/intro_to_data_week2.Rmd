...
ggplot(nycflights, aes(x = factor(month), y = dep_delay)) +
  geom_boxplot(outlier.colour = NA) + coord_cartesian(ylim= c(-10,75))
```
...
nycflights <- nycflights %>%
  mutate(avg_speed = nycflights$distance / (air_time/60))
nycflights[,c('avg_speed','tailnum')] %>%
...
ggplot(data=nycflights, aes(x=nycflights$distance, y=avg_speed)) + geom_point()
```
...
ot_arrival_prop <- sum(nycflights$arr_type == 'on time') / length(nycflights$arr_type)
ot_arrival_prop
ot_arrival_dl_dep <- sum(nycflights$dep_type == 'delayed' & nycflights$arr_type == 'on time') / length(nycflights$arr_type)
ot_arrival_dl_dep
```